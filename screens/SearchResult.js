...
import { View, Text, StyleSheet, SafeAreaView, TextInput, ScrollView, AsyncStorage } from 'react-native';
...
class SearchResult extends Component {
  ...
  render() {
    ...
    const result =  this.state.searchCafeList.length !== undefined ? this.state.searchCafeList.slice(0,10).map( (ele, idx) => <SuggestCafeListEntry key={idx} cafe={ele} handlePress={this.props.navigation.getParam('handlePress')} /> ) : <Text>Loading</Text>
    return (
        ...
        <View style={ styles.container}>
          <View style={ styles.explore}>
            <View style={ styles.searchBarConatiner}>
              ...
              <TextInput value={this.state.target} placeholder="Search Awesome Cafe" placeholderTextColor="grey" onSubmitEditing={this.requestSearchResult.bind(this)} onChangeText={(target)=>{this.setState({target})}} style={styles.searchBar} />
            </View>
          </View>
          <ScrollView>
            { result }
